...
function drawStatBox(ctx: CanvasRenderingContext2D, rect: Rect, icon: 'trophy' | 'clock', label: string, value: string) {
    ...
    if (icon === 'trophy')
        drawTrophyIcon(ctx, iconCx, iconCy, iconSize);
    else
        drawClockIcon(ctx, iconCx, iconCy, iconSize);
...
function drawButton(ctx: CanvasRenderingContext2D, rect: Rect, label: string, active: boolean, hovered: boolean) {
...
function drawScrollButtons(ctx: CanvasRenderingContext2D, nav: SongSelectNavState, totalSongs: number) {
    if (!nav.layout.showScrollArrows)
        return;
...
export function drawSongSelect(ctx: CanvasRenderingContext2D, width: number, height: number, nav: SongSelectNavState, previewingId: string | null, previewPaused: boolean): void {
