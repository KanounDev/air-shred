...
export function StatusBar({ stateRef }: Props) {
    ...
    return (<div className="status-bar">
      ...
      {s.liveMatchOctave !== null && s.liveMatchOctaveDist !== null && (<span className="status-item status-item--dim">
          ~Oct{s.liveMatchOctave + 1} (d={s.liveMatchOctaveDist.toFixed(2)})
        </span>)}

      {showFired ? (<span className="status-item status-item--note-fired">NOTE {s.lastNotePlayed}</span>) : (s.liveMatchNote !== null &&
            s.liveMatchDist !== null && (<span className="status-item status-item--dim">
            ~{NOTE_NAMES[s.liveMatchNote]} (d={s.liveMatchDist.toFixed(2)})
          </span>))}

      {s.gateEnabled && (<span className={`status-item ${s.gateArmed ? 'status-item--gate-armed' : 'status-item--gate-blocked'}`}>
