...
export function useSongGame() {
  ...
  const registerPlayedNote = useCallback((noteIndex: number, octave: number) => {
    const state = stateRef.current;
    if (!state.running || !state.currentTarget || state.finished) return;
  ...
  const tick = useCallback(() => {
    const state = stateRef.current;
    if (!state.running || !state.activeSong) return;
    ...
    // Only advance to the next target once the current note's window has
    // passed AND the player has matched the pose. This enforces the
    // requirement: wait for the player's pose before moving on.
    const targetEnd = state.currentTarget.time + state.currentTarget.duration + 200;
    if (state.elapsedMs >= targetEnd) {
      // advance only if matched; otherwise keep waiting until matched
