...
export default function App() {
    ...
    const { stateRef: menuNavStateRef, processFrame: processMenuFrame, reset: resetMenuNav } = useMenuNavigation((id) => {
        if (id === 'training')
            enterTraining();
        else if (id === 'tutorial')
            openTutorial();
    ...
    const { stateRef: gameStateRef, loadSong, endGame, registerPlayedNote, tick: tickGame, resetGame, } = useSongGame();
    const lastNoteTimeRef = useRef(0);
    const handleStartSong = useCallback(async (songId: string) => {
    ...
    const startPreview = useCallback(async (songId: string) => {
        if (!audioEngine)
            return;
        stopPreview();
        const song = findSong(songId);
    ...
    const handlePreview = useCallback((actionId: string) => {
        if (actionId === 'pause') {
            pausePreview();
            return;
        }
    ...
    const backToMenu = useCallback(() => {
        ...
        const blockedId = screenRef.current === 'training' ? 'training' : screenRef.current === 'songSelect' ? 'play' : null;
        if (screenRef.current === 'songSelect')
            resetSongSelectNav();
    ...
    const handleFrame = useCallback((frame: HandFrameResult) => {
        const s = screenRef.current;
        if (s === 'menu') {
            if (!tutorialOpenRef.current)
                processMenuFrame(frame);
        }
    ...
    return (<div className="app">
      <main className="stage-wrap">
        <CameraCanvas videoRef={videoRef} canvasRef={canvasRef} status={status} error={error} onStart={handleStart}/>
        {trackingReady && screen === 'menu' && !tutorialOpen && (<MenuOverlay navStateRef={menuNavStateRef} trackingReady={trackingReady} active/>)}
        {trackingReady && screen === 'training' && <StatusBar stateRef={gestureStateRef}/>}
        {trackingReady && screen === 'songSelect' && (<SongSelectOverlay navStateRef={songSelectNavStateRef} active previewingId={previewingId} previewPaused={previewPaused}/>)}
        {tutorialOpen && <TutorialModal onClose={closeTutorial}/>}
      </main>

      {trackingReady && (screen === 'training' || screen === 'play') && (<section className="piano-wrap">
